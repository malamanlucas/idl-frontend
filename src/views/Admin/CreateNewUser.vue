    ...
    <v-col>
      <v-form>
        ...
        <v-text-field :type="showPassword ? 'text' : 'password'"
            ...
            label="Password" v-model="password" placeholder="Exemplo: 123"></v-text-field>
    ...
    methods: {
      async save() {
        ...
        if (!this.$v.$invalid) {
          try {
            this.showLoading()
            await adminService.createUserAndPassword(this.username, this.password)
            this.showSuccessOnSnackbar('Usuário e senha criado com sucesso!')
          } catch (e) {
